# NOEZEMA — Архив STATUS.md: закрытые разделы вех

Создан T7.42 (2026-09-25, исходный коммит `747f10b`). Файл — побайтовый (дословный) перенос
закрытых разделов `docs/STATUS.md`: Gate M2; Gate M3 (включая историю M4 — T4.1–T4.9 + Gate M4);
M5 (включая M6 — T6.1–T6.4 + Gate M6); M7 — T7.1–T7.28 (follow-up); плюс исходные ячейки
«Статус»/«Примечание» таблицы вех M0–M7, сжатые в новом STATUS.md. Ни одна строка перенесённых
разделов не переписана и не сокращена; навигация — по оглавлению ниже (якоря GitHub-формата)
или grep'ом. Вмятые псевдозаголовки (T7.15–T7.16, T7.19–T7.21) перенесены вместе со своим
разделом.

Это второй по приоритету источник для расследования старых решений и закрытых вех (ниже
основного `docs/STATUS.md`; порядок приоритетов — AGENTS.md §1). Активная история (T7.29–T7.41)
в этом файле отсутствует — она в `docs/STATUS.md`.

## Оглавление

- [Таблица вех — исходные ячейки «Статус»/«Примечание» (дословно)](#таблица-вех--исходные-ячейки-статуспримечание-дословно)
  - [M0 каркас](#m0-каркас)
  - [M1 контракты + LLM](#m1-контракты--llm)
  - [M2 изоляция + commit](#m2-изоляция--commit)
  - [M3 память + web slice (MVP)](#m3-память--web-slice-mvp)
  - [M4 зависимости + переоценка](#m4-зависимости--переоценка)
  - [M5 расширенный цикл](#m5-расширенный-цикл)
  - [M6 Research Proxy](#m6-research-proxy)
  - [M7 полный веб + эксплуатация](#m7-полный-веб--эксплуатация)
- [Gate M2 (§19, этап 2) — пройден (noezema-m2)](#gate-m2-19-этап-2--пройден-noezema-m2)
- [Gate M3 (этап 3a + MVP-критерии §22.1)](#gate-m3-этап-3a--mvp-критерии-221)
- [Закрыто T3.29 (пункт 1 §22.1)](#закрыто-t329-пункт-1-221)
- [Закрыто T3.30 (дефект из первой реальной MVP-сессии)](#закрыто-t330-дефект-из-первой-реальной-mvp-сессии)
- [M5. Расширенный познавательный цикл (этап 4)](#m5-расширенный-познавательный-цикл-этап-4)
- [Gate M6 (§19, этап 5) — пройден](#gate-m6-19-этап-5--пройден)
- [M7. Полный веб + эксплуатация (этапы 6-full + 7)](#m7-полный-веб--эксплуатация-этапы-6-full--7)
- [T7.13 — `message.reply` не предлагать при пустом inbox + самоочевидная схема (EVAL-3b P.6)](#t713--messagereply-не-предлагать-при-пустом-inbox--самоочевидная-схема-eval-3b-p6)
- [T7.14 — правила в промптах (EVAL-3b P.2/P.4)](#t714--правила-в-промптах-eval-3b-p2p4)
- [T7.17 — устойчивый scope: оценка по хост-деривации, rules-v2 (EVAL-3/T7.15, §3.7, §8.7, §11.2)](#t717--устойчивый-scope-оценка-по-хост-деривации-rules-v2-eval-3t715-37-87-112)
- [Merge T7.8–T7.17 в `main` (2026-09-18)](#merge-t78t717-в-main-2026-09-18)
- [T7.18 — относительная опорная дата выводится хостом (шаг 1 перезапуска EVAL-3c, ADR-0007, §3.7, §8.7)](#t718--относительная-опорная-дата-выводится-хостом-шаг-1-перезапуска-eval-3c-adr-0007-37-87)
- [Merge T7.18–T7.20 в `main` (2026-09-20)](#merge-t718t720-в-main-2026-09-20)
- [T7.22 — окно assertion-фрагмента: второе (value) окно для группы A EVAL-3d (§6.4, ADR-0011)](#t722--окно-assertion-фрагмента-второе-value-окно-для-группы-a-eval-3d-64-adr-0011)
- [T7.23 — совместимость структурированного вывода с движками, не принимающими часть ключевых слов JSON Schema (halogen: format/pattern), fail-closed audit (§5.2.2, §6.5, ADR-0012)](#t723--совместимость-структурированного-вывода-с-движками-не-принимающими-часть-ключевых-слов-json-schema-halogen-formatpattern-fail-closed-audit-522-65-adr-0012)
- [T7.24 — обрыв EVAL-4 (2026-09-21): commit-boundary dead end — reconcile-tick + staging-последовательность + known rollback → terminal (§5.2.2, §6.5, §6.7)](#t724--обрыв-eval-4-2026-09-21-commit-boundary-dead-end--reconcile-tick--staging-последовательность--known-rollback--terminal-522-65-67)
- [T7.25 — переиспользование между днями claim'а с относительной датой: решение (а) — корректный fail-closed, не дефект (ADR-0007, §3.7, §8.2, §8.6, §8.7, §11.3)](#t725--переиспользование-между-днями-claimа-с-относительной-датой-решение-а--корректный-fail-closed-не-дефект-adr-0007-37-82-86-87-113)
- [Корпус v3 (EVAL-4) — 2026-09-20](#корпус-v3-eval-4--2026-09-20)
- [T7.26 — drain-протокол online-активации: ожидание «окна» в живой серийной серии без гонки T7.20 (EVAL-4d, §8.7.2, уточнение ADR-0009 — ADR-0013)](#t726--drain-протокол-online-активации-ожидание-окна-в-живой-серийной-серии-без-гонки-t720-eval-4d-872-уточнение-adr-0009--adr-0013)
- [T7.27 — свежесть claim'ов: правило §8.6/T3.7 вычисляется на момент чтения в гейте и retrieval; сохранённое поле — дисплейный кэш (EVAL-4d, исправление под спецификацию — ADR-0014)](#t727--свежесть-claimов-правило-86t37-вычисляется-на-момент-чтения-в-гейте-и-retrieval-сохранённое-поле--дисплейный-кэш-eval-4d-исправление-под-спецификацию--adr-0014)
- [T7.28 — гейт `due_stale_time_sensitive`: строгое направление `<` по спецификации (ADR-0015, уточнение ADR-0004)](#t728--гейт-due_stale_time_sensitive-строгое-направление--по-спецификации-adr-0015-уточнение-adr-0004)
- [T7.28 (follow-up) — направления гейтов — закрытый набор: `GateDirection` Literal + fail-closed в `_gate()`](#t728-follow-up--направления-гейтов--закрытый-набор-gatedirection-literal--fail-closed-в-_gate)

## Таблица вех — исходные ячейки «Статус»/«Примечание» (дословно)

Дословный текст ячеек таблицы «Прогресс по вехам» из `docs/STATUS.md` (до T7.42) для вех M0–M7;
в новом STATUS.md эти ячейки сжаты до одной строки сути + ссылки на этот раздел. Строка не
переписана и не сокращена.
