...
| # | Задача |
|---|---|
| T5.1 ✅ | Curiosity ranking (§5.3.1): score-формула, eligibility filter, нормализация, ε-разнообразие; веса/пороги/embedding fingerprint — в config snapshot. Тесты: packages/cognition/curiosity.py — score = w1*novelty + w2*coverage_gap + w3*evidenceability + w4*feasibility − w5*cost − w6*risk − w7*topic_recency, все входы [0,1], сохраняются в questions.score_components + embedding_fingerprint (token-jaccard-v1 — similarity при выключенных embeddings, pgvector ADR-gated); novelty против (прошлые вопросы ∪ claim statements) — гасит «новизну через перефразирование»; coverage_gap 1.0 для origins-слабостей (conflict/unverified/invalid/unknown_term), иначе доля открытого долга; evidenceability по origin (конкретный источник 1.0 / общий путь 0.5); feasibility = word-лимит контекста; cost = длина формулировки; risk 0.0 (sealed profile, риск-модель — отдельный ADR); topic_recency — доля последних R сессий с пересечением темы; eligibility filter до ранжирования; ε-diversity: argmax с 1−ε, иначе равномерный выбор из top-M ∪ (max−δ), seed = sha256(session+candidates) в audit (воспроизводимость); веса/пороги/ε/M/δ/recency/fingerprint — в curiosity-секции snapshot (bootstrap: fifo), malformed → fail-closed CuriosityConfigError; селектор config-driven в оркестраторе (curiosity.selector), score в audit question_selected.payload.curiosity. tests/unit/test_curiosity.py (15) + tests/scenario/test_curiosity_selector.py (3: ranking бьёт FIFO; ε-выбор в пуле top-2 воспроизводим по seed; полный оркестратор — online config change переключает ранжирование) |
| T5.2 ✅ | Многошаговое planning: план как наблюдаемый артефакт (наблюдения, критерии остановки, assessment methods). Тесты: packages/domain/schemas/plan.py + planning-фаза оркестратора — закрытая структура плана (steps {observation, method, tool_hint?}, stopping_criteria, assessment_methods — закрытый хостовый enum recompute/cross_source_check/rules_only/no_change; метод проверки отдельное поле, не перефраз — критерий Gate M5; план без grade/confidence и без evidence — host-owned boundary не тронута); роль planner (prompts/planner.md planner-v1) — LLM-вызов в planning-фазе, model_runs phase=planning; host-валидация схема + planning.max_steps из snapshot; невалидный/сверхбюджетный план → fallback на MVP template + audit plan_fallback (schema_invalid/budget_exceeded), сессия не падает; наблюдаемость: sessions.plan JSONB + plan_sha256 (canonical, миграция 0013, пишется один раз) + audit plan_proposed; отрендеренный план в explorer-контекст; config planning.mode template|llm (bootstrap template, NULL=template, online-активация наследует) — включение online config change. tests/unit/test_plan_schema.py (11) + tests/scenario/test_planning.py (4: полный цикл persist+audit+model_runs; schema-invalid fallback; budget fallback; template-mode без planner-вызова) |
| T5.3 ✅ | Специализированные роли explorer/verifier/curator: verifier организует детерминированные проверки, **не назначает grade** (§5.5, §3.7). Тесты: packages/domain/schemas/verification.py + verifying-фаза оркестратора — роль verifier (prompts/verifier.md verifier-v1) без инструментов, LLM-вызов в verifying-фазе, model_runs phase=verifying; отчёт {checks [{description, method, evidence_indexes (ссылки на зафиксированное evidence), result pass|fail|not_applicable, note?}], gaps} — closed schema extra=forbid, **структурно без полей grade/confidence/status (gate, тест по model_fields)**; host-валидация schema + verification.max_checks + referential; невалидный/сверхбюджетный/висящий → fallback на MVP no-op + audit verification_fallback, сессия не падает; наблюдаемость sessions.verification + verification_sha256 (canonical, миграция 0014) + audit verification_completed; отчёт — предложение в curator-контекст (пома́тка «не оценка»); **сценарий: assessment claim (grade, confidence, epistemic_status) идентичен с verifier и без него — rules engine не тронут**; config verification.mode off|llm (bootstrap off, NULL=off, online-наследование, включение online config change). tests/unit/test_verification_schema.py (8) + tests/scenario/test_verification.py (5) |
| T5.4 ✅ | Защита от семантических повторов (§9): similarity целей, перефразирование, циклы планов, стратегии при цикле. Тесты: packages/cognition/repetition.py + guard в выборе вопроса оркестратора — детектор (детерминированный, Jaccard по word set, fingerprint repeat-jaccard-v1): перефраз уже исследованного вопроса (rephrase_threshold из конфига) + no_progress последовательных сессий без нового claim (сессия с claim сбрасывает счётчик) ≥ no_progress_limit → цикл; стратегия из **закрытого списка §9** (детерминированная ротация, не случайный толчок): compare_previous_session/opposite_hypothesis/change_source_type/experiment → host-заметка в explorer-контекст; defer_question/choose_different_area → вопрос deferred + следующий кандидат (selector exclude_ids); audit repeat_cycle_detected (similarity, no_progress, strategy, fingerprint) + question_deferred; guard только для автономного выбора; config repetition (enabled, пороги, лимит) — bootstrap disabled (MVP не меняется), NULL=disabled (fail-closed), online config change (миграция 0015). v1: план-циклы — follow-up (план — артефакт T5.2; plan_cycle_threshold зарезервирован). tests/unit/test_repetition.py (7) + tests/scenario/test_repetition_guard.py (5) |
| T5.5 ✅ | Untrusted extraction profile: модель без инструментов извлекает chunks с provenance из высокорисковых документов. Тесты: packages/domain/schemas/extraction.py + extraction-профиль в tool-цикле оркестратора — workspace.read ≥ extraction.min_document_bytes → роль extractor (prompts/extractor.md extractor-v1) без инструментов (tool_schema_hash([])), документ в запросе отделён data-boundaries UNTRUSTED DATA; модель — только дословные цитаты; **host: генерация provenance (path, document_sha256, quote_sha256, index) + проверка дословности + дедупликация + бюджет max_chunks (gate: raw-текст не покидает extractor — сценарий по request-логу fake LLM)**; explorer получает только chunks с provenance (пометка «недоверенные данные»); fallback на raw read при schema/verbatim/budget (audit extraction_fallback, сессия не падает); sessions.extraction + extraction_sha256 (canonical, миграция 0016; сырой контент не хранится) + audit extraction_completed + model_runs; extraction.mode off|llm (bootstrap off, NULL=off, online change); fake-сервер логирует last-user (GET /_noezema/requests). tests/unit/test_extraction_schema.py (8) + tests/scenario/test_extraction.py (4) |
| T5.6 ✅ | Long-run сценарные тесты (мульти-сессионные, с повторами и противоречиями). Тесты: tests/scenario/test_long_run.py (3) — система как long-running thinker, реальные оркестраторские сессии против одной БД: (1) накопление знания — 4 сессии, FIFO-очередь, каждый claim переживает следующие сессии (head current supported E2, ровно одна assessment), новый вопрос сессии (previous_result) встаёт в очередь и берётся следующей; (2) §9-цикл на накопленной истории — верифицированный вопрос + 2 no-progress + перефраз → детерминированная стратегия compare_previous_session, audit repeat_cycle_detected, host-нота реально в контексте explorer (request-лог fake LLM), сессия делает прогресс (claim); (3) поздний контрпример — сессия 2 с тем же claim (dedup) + counters-evidence → rules engine переоценивает: disputed E1 (counterevidence_unresolved, §3.7), grade не от модели |
...
| T7.8 | EVAL-3b post-mortem (P.1, §6.4, §14.3): payload evidence `source_assertion` несёт **текст утверждения** из chunk (явный бюджет фрагмента), чтобы куратор формулировал external/temporal_fact по содержимому, а не только по URL и хэшами; identity продолжает считаться по хэшу исходного содержимого (семантика дедупликации не меняется) | ⬜ |
| T7.9 | EVAL-3b post-mortem (P.2, §14.1, §14.3): **claim без head не коммитится** — предложение куратора, отклоняемое rules engine (claim_type ↔ allowed evidence kinds), отбивается ДО commit, а не превращается в problems при продолжении; инвариант «каждый committed claim имеет head активного snapshot» + сценарный тест; dedup не переиспользует существующий claim без head | ⬜ |
| T7.10 | EVAL-3b post-mortem (P.3, §5.4, §11.2): обрезка explorer-контекста отбрасывает **самые старые** наблюдения, а не хвост — результат последнего `research.fetch` обязан присутствовать в промпте шага (тест это доказывает) | ⬜ |
| T7.11 | EVAL-3b post-mortem (P.3, §11.3): **идемпотентный refetch** research proxy — при уже существующем артефакте по хэшу возвращать существующие source и chunk, а не 500 (UniqueViolation на `artifact_chunks`) | ⬜ |
| T7.12 | EVAL-3b post-mortem (P.3, §5.7, §9): **intra-session repeat guard** — N одинаковых (инструмент, хэш аргументов) в сессии → deny с наблюдением, объясняющим модели, что результат уже есть | ⬜ |
| T7.13 | EVAL-3b post-mortem (P.4, §3.7, §11.2): `message.reply` — **inbox-gating**: при пустом inbox инструмент не предлагается модели (или схема сделана самоочевидной + объясняющий deny) — 3 потерянных шага из 10 в двух сессиях EVAL-3b | ⬜ |
| T7.14 | EVAL-3b post-mortem (P.1/P.3, §5.5, §11.2): правила в промптах как подстраховка — explorer: не более двух повторов после ошибки инструмента, затем смена стратегии; curator: если в evidence нет текста утверждения — создавать вопрос, а не claim (+ обновление prompt pins) | ⬜ |
| T7.15 | EVAL-3b post-mortem (условие перезапуска, §22.2): **E2E-валидация на черновой БД** — 2–3 собственных URL-вопроса (НЕ замороженный корпус): research.fetch → source_assertion с текстом → claim temporal_fact/external_fact → head current → grade E3 по двум независимым источникам; выдержка из БД (тип, statement, epistemic_status, grade, evidence) — до этого EVAL-3 не перезапускается | ⬜ |
| T7.16 | EVAL-3b (остаточный дефект P.1, §6.4): **вопрос-зависимый выбор фрагмента assertion_text** — окно вокруг места с наибольшей плотностью терминов вопроса/плана в нормализованном тексте (чистый модуль apps/orchestrator/assertion_window.py; бюджет 2000 символов не поднят; identity §14.3 не меняется; fallback — ведущий префикс T7.8) | ✅ |
| T7.17 | EVAL-3 (T7.15, §3.7, §8.7, §11.2): **устойчивый scope** — покрытие scope не зависит от того, придумает ли модель одинаковые ключи: scope, который проверяет rules engine, выводит доверенный хост (claim scope — из вопроса: опорная дата + названные источники; evidence scope — из provenance: registrable domain + retrieved_at), модельный free-form scope игнорируется для покрытия (остаётся в staging/аудите); fail-closed: источник не назван вопросом / retrieved раньше опорной даты → E1, grade не поднимается; legacy-scope — исходный key-by-key предикат (апгрейд не переоценивает знание); rules-v1 → rules-v2; ADR-0007 | ✅ |

**Gate M7** (§19, этап 7): §22.1 полный + §22.2 пройден (ни один gate не `failed` и не
`insufficient_sample`).

**Задачи T7.8–T7.17** (дефекты, найденные оценкой EVAL-3b — работа над системой по §22.2,
не разовые правки): постмортем — `docs/eval/EVAL-3b-postmortem.md`; правки — логическая
последовательность PR'ов T7.8 → T7.14, E2E-валидация T7.15, остаточный дефект выбора
фрагмента T7.16, устойчивый scope (хост-деривация, rules-v2, ADR-0007) — T7.17. Замороженный
корпус, конфиги v2/v3 и пороги §22.2 не меняются (хэши прежние); новый прогон не
