...
> **Пометка (2026-09-22, T7.28/ADR-0015; исторические значения не
> пересчитаны):** формулировка спеки у гейта due/stale — строго
> «<20%» (`ARCHITECTURE.md:2607`); код с T7.28 приведён к спецификации
> (строгое `ratio < threshold`). Строка «≤0.20 / +1 due ещё пройдёт
> (0.20)» выше описывает поведение кода на момент заморозки (`at_most`:
> ровно 0.20 проходило). При строгом сравнении 3/20 = 0.15 всё равно
> passed, но запас «+1 due» исчезает: 4/20 = 0.20 → failed.
...
> **Пометка (T7.35, ADR-0019, 2026-09-23):** замороженные payload'ы
> v2/v3 объявляют `curator-v2/explorer-v2` как ЯРЛЫКИ (содержимое не
> зафиксировано, путь из payload'а загрузчиком игнорировался); код
> `83d0ea9` уже включал промпты T7.14 — **фактически EVAL-3c/3d шло с
> `curator-v3/explorer-v3`**. См. ADR-0019, таблица криминалистики.
