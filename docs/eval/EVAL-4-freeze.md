# EVAL-4 — замороженная конфигурация (подготовлено 2026-09-20; запущено: EVAL-4c остановлен, EVAL-4d пройден до конца)

Статус: **подготовка завершена 2026-09-20; EVAL-4c (2026-09-21 19:06 UTC)
запущен и остановлен через ~1 минуту (§8, T7.25); EVAL-4d (2026-09-21
20:33 UTC) пройден до конца — 69/69 сессий, mid-run флип не состоялся,
outcome=failed (§9, T7.26); гейт 6 по правилу — failed 22/28 (T7.27,
«Постскриптум» в §9 ниже). Решение о новом прогоне — за пользователем.**
Ограничение «утренний старт» (§6 п.8) осталось в силе.
...
> **Пометка (T7.35, ADR-0019, 2026-09-23):** v4/v5 объявляют
> `curator-v2/explorer-v2` как ЯРЛЫКИ — содержимое не зафиксировано
> (путь из payload'а загрузчиком игнорировался, версия не сверялась со
> snapshot'ом). **Фактически вся серия EVAL-4 (4/4b/4c/4d) шла с
> `curator-v3/explorer-v4`** — промптами T7.14/T7.21, которые были на
> диске кода `ff59dbf`. См. ADR-0019, таблица криминалистики.
> Payload-файлы не переписаны; исполняемыми с T7.35 являются только
> payload'ы с пином содержимого (v7+).
...
- **Гейт 6 (`due_stale_time_sensitive`) — предрегистрация ожидания: при
  N ≤ 30 текущих temporal-claim'ов — `failed` по дизайну корпуса, даже при
  безупречном поведении модели (код с T7.28/ADR-0015 — строгий `<` по
  спецификации).** Корпус v3 намеренно содержит 6 вопросов
  ...
  Порог гейта: по спецификации — строго меньше 20%
  (`ARCHITECTURE.md:2607`, «due/stale time-sensitive claims <20%»); код
  приведён к спецификации T7.28 (ADR-0015: строгое направление `below` —
  `ratio < threshold`, порог 0.20 не тронут). Отсюда минимальный
  знаменатель, при котором 6 заложенных просрочек не валят гейт сами по
  себе: **N ≥ 31** (6/30 = 0.200 → failed; 6/31 = 0.1935 → passed).
  ...
  **Для приёмки**: красный g6 на корпусе v3 при N ≤ 30 не является
  измерением того, насколько система следит за свежестью знания. Чтобы гейт стал измерением, нужно одно из двух (решение
...
### Постскриптум (T7.27, 2026-09-22): гейт 6 по правилу

Сохранённый итог `due_stale_time_sensitive` (passed 0/28) — артефакт
старой реализации (читала дисплейный кэш `freshness_status`, переоценка
не запускалась — T7.26/ADR-0013). По правилу §8.6/T3.7 на момент
расчёта гейта (T7.27/ADR-0014, строгое направление T7.28/ADR-0015):
**failed 22/28** (ratio 0.7857 при пороге 0.20) — 16 модельных as_of-
артефактов + 6 заложенных корпусом old-as_of (группа B, §3.1). Разбор
as_of (семантика по спеке, путь значения, улика по 22 claim'ам,
сверка с EVAL-3d, варианты решения и пересчёт due) —
[EVAL-4d-as-of-analysis.md](EVAL-4d-as-of-analysis.md) (T7.29).
Строка прогона и сохранённые итоги не пересчитаны (SELECT only).
