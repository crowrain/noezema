# NOEZEMA — Architecture Draft

> Status: draft v0.2  
> Language: Russian  
> Purpose: describe the target architecture of a local-first autonomous thinker focused on curiosity, verifiable learning, persistent memory, safe action, and human-observable operation.

## 1. Идея проекта

NOEZEMA — автономный локальный мыслитель, который периодически пробуждается, выбирает неизвестный ему вопрос, исследует его, проверяет выводы, сохраняет знания с доказательствами и оставляет понятный отчёт для следующей сессии и человека-наблюдателя.

Проект сохраняет сильные стороны эксперимента `ai_lives_on_computer`:

- дискретные сессии вместо непрерывного процесса;
- преемственность через внешнюю память;
- собственное рабочее пространство;
- свободу выбора интересов и направления исследования;
- наблюдаемость действий;
- возможность развивать идентичность и внутренние правила.

При этом NOEZEMA устраняет основные архитектурные проблемы исходного подхода:

- управляющий контур отделён от среды мыслителя;
- модель не исполняет команды непосредственно на хосте;
- действия передаются через типизированный протокол;
- знания отделены от субъективных воспоминаний;
- утверждения связаны с доказательствами и статусом проверки;
- история событий неизменяема;
- повторения определяются семантически, а не по хешам файлов;
- локальная LLM является основным, а не дополнительным режимом;
- текущее состояние и хронология доступны через веб-интерфейс.

## 2. Цели

### 2.1. Основные цели

1. Работать с локальной LLM через OpenAI-compatible API.
2. Накапливать новое для агента знание, а не только автобиографические заметки.
3. Хранить происхождение, доказательства и контраргументы для каждого значимого утверждения.
4. Выполнять действия только в изолированной среде.
5. Быть наблюдаемым и восстанавливаемым после сбоев.
6. Поддерживать общение с человеком без превращения каждого сообщения в безусловную команду.
7. Не зависеть от конкретной модели, поставщика или inference backend.

### 2.2. Не-цели первой версии

- доказательство наличия сознания или субъективного опыта;
- неограниченный доступ к хостовой системе;
- выполнение произвольных внешних действий от имени владельца;
- multi-agent orchestration ради самой сложности;
- обучение или дообучение основной LLM во время работы;
- публичное раскрытие скрытой chain of thought модели.

## 3. Архитектурные принципы

### 3.1. Свобода внутри ограниченного мира

Мыслитель свободен выбирать темы, создавать проекты, менять внутреннюю идентичность и организовывать своё рабочее пространство. Границы безопасности, планировщик, журнал событий и резервные копии находятся вне его контроля.

### 3.2. Управление отделено от мышления

LLM предлагает намерение и действие. Orchestrator и Policy Engine решают, можно ли выполнить действие и в какой среде.

### 3.3. События первичны, представления вторичны

Все значимые переходы записываются в append-only журнал. Текущий статус, веб-хронология и отчёты являются проекциями этого журнала.

### 3.4. Знание требует происхождения

Запись модели не становится фактом автоматически. Семантическая память хранит статус утверждения, уверенность, источники, контраргументы и историю проверок.

### 3.5. Локальная модель — штатный режим

LLM Gateway проектируется вокруг локального OpenAI-compatible endpoint. Поддержка удалённых моделей может быть добавлена позднее как сменный профиль.

### 3.6. Наблюдаемость без публикации скрытых рассуждений

Полный ответ модели, включая рассуждения, сохраняется в event store с ограниченной видимостью: без него невозможны отладка, аудит и разбор неудачных сессий. Публично сайт показывает вопросы, планы, краткие мотивировки, действия, результаты, источники и выводы.

Различие проходит по полю `visibility`, а не по факту записи. Скрытые рассуждения не публикуются как «поток сознания» и не являются источником утверждений: статус claim определяется доказательствами, а не текстом размышления.

### 3.7. Проверка опирается на детерминированные основания

Вывод второй LLM-роли сам по себе верификацией не является. Изменить статус утверждения может только детерминированная проверка: исполнение кода, сверка цитаты с сохранённым текстом источника, сопоставление независимо полученных источников, вычисление. Роль verifier организует такие проверки и интерпретирует результат, но не заменяет их.

## 4. Контекст системы

```mermaid
flowchart TB
    subgraph Host["Доверенный контур — отдельная Linux VM"]
        Supervisor["systemd<br/>запуск и восстановление"]
        Orchestrator["Session Orchestrator<br/>машина состояний"]
        Curiosity["Curiosity Engine<br/>выбор вопроса"]
        Context["Context Builder<br/>сбор контекста"]
        Gateway["LLM Gateway<br/>OpenAI-compatible API"]
        Memory["Memory Service<br/>поиск и консолидация"]
        Policy["Policy Engine<br/>проверка действий"]
        ToolBroker["Tool Broker<br/>типизированные инструменты"]
        EventStore[("PostgreSQL<br/>сессии, события, память")]
        Artifacts[("Artifact Store<br/>файлы, код, отчёты")]
        Model["Локальная LLM<br/>llama.cpp / Ollama / vLLM"]
        FetchProxy["Research Proxy<br/>контролируемое чтение интернета"]
    end

    subgraph Sandbox["Одноразовый sandbox мыслителя"]
        Workspace["/workspace<br/>проекты и заметки"]
        Shell["Ограниченный shell"]
        Python["Python / notebooks"]
    end

    subgraph Site["Веб-модуль"]
        Dashboard["Текущий статус"]
        Timeline["Хронология"]
        Knowledge["Карта знаний"]
        Messages["Сообщения мыслителю"]
        Controls["Пауза / пробуждение"]
    end

    Supervisor --> Orchestrator
    Orchestrator --> Curiosity
    Curiosity --> Context
    Context --> Memory
    Context --> Gateway
    Gateway --> Model
    Gateway --> Policy
    Policy --> ToolBroker
    ToolBroker --> Sandbox
    ToolBroker --> FetchProxy
    Sandbox --> Artifacts
    FetchProxy --> Artifacts

    Orchestrator --> EventStore
    Memory <--> EventStore
    ToolBroker --> EventStore
    Site <--> EventStore
    Site --> Artifacts
    Messages --> Orchestrator
```

## 5. Компоненты

### 5.1. Supervisor

Отвечает только за жизненный цикл сервисов:

- запуск Orchestrator и веб-приложения;
- автоматическое восстановление после падения;
- корректное завершение;
- передачу минимальной конфигурации;
- health checks.

Предпочтительная реализация для одного узла — `systemd`.

### 5.2. Session Orchestrator

Центральная машина состояний. Orchestrator:

- определяет момент пробуждения;
- создаёт сессию и lease;
- фиксирует версии модели, промптов и политик;
- вызывает фазы познавательного цикла;
- контролирует бюджеты времени, шагов и токенов;
- завершает сессию транзакционно;
- создаёт checkpoint;
- восстанавливает зависшие сессии.

Orchestrator не должен быть доступен для изменения из sandbox.

#### 5.2.1. Расписание пробуждений

Расписание принадлежит доверенному контуру и недоступно из sandbox.

- Базовое расписание задаётся cron-подобным выражением с минимальным интервалом между сессиями.
- Перед запуском проверяются условия допуска: нет незавершённой сессии с живым lease, свободна GPU-память под профиль модели, соблюдена дисковая квота, система не на паузе.
- Если условия не выполнены, пробуждение пропускается с записью причины, а не ставится в очередь: накопленная очередь превращается в лавину сессий после простоя.
- После неудачной сессии применяется экспоненциальный backoff; после нескольких неудач подряд система переходит в `paused` и ждёт человека.
- Ручное «пробуждение сейчас» обходит расписание, но не условия допуска.

### 5.3. Curiosity Engine

Поддерживает реестр исследовательских вопросов и выбирает следующий вопрос по сочетанию факторов:

- новизна для текущей памяти;
- ожидаемый прирост информации;
- наличие проверяемых источников или эксперимента;
- отличие от последних тем;
- выполнимость в текущем бюджете;
- связь с долгосрочными интересами;
- риск и стоимость.

Источники кандидатов:

- противоречия в памяти;
- неизвестные термины;
- непроверенные утверждения;
- результаты предыдущих экспериментов;
- локальный корпус;
- сообщения человека;
- случайное тематическое исследование;
- предложение самой модели.

#### 5.3.1. Baseline первой версии

Перечисленные факторы — исследовательская задача, а не спецификация. Чтобы этап не стал бесконечным, первая версия реализует намеренно примитивный baseline:

```text
novelty(q) = 1 - max cosine_similarity(embed(q), embed(claim_i)) по k ближайшим claims
recency(t) = доля последних R сессий, посвящённых теме t
score(q)   = w1*novelty + w2*feasibility - w3*cost - w4*recency(topic(q))
```

- «Ожидаемый прирост информации» в v1 не оценивается. Вместо него — булев фильтр: у вопроса должен быть проверяемый источник или воспроизводимый эксперимент.
- Выбор: `argmax score` с вероятностью `1 - ε` и случайный допустимый кандидат с вероятностью `ε ≈ 0.2`. Это даёт тематический дрейф без отдельной эвристики разнообразия.
- Веса и `ε` хранятся в конфигурации и версионируются вместе с политиками, чтобы результат сессии оставался воспроизводимым.
- Усложнение модели любопытства допускается только после того, как baseline даст измеримую базовую линию по метрикам §16. Иначе улучшение неотличимо от шума.

### 5.4. Context Builder

Формирует ограниченный context pack вместо передачи всей истории. В него входят:

- актуальная версия идентичности;
- правила среды и протокол действий;
- текущий вопрос;
- итог последней сессии;
- релевантные утверждения и доказательства;
- незакрытые противоречия;
- новые сообщения;
- недавние ошибки и повторения.

Поиск гибридный: полнотекстовый, embedding similarity, свежесть, значимость и связь с вопросом.

#### 5.4.1. Токенные бюджеты

Качество работы определяется не «мышлением», а тем, что попало в context pack. Поэтому пакет собирается по фиксированным бюджетам, а не по принципу «сколько поместится»:

```text
протокол, инструменты, правила среды  — жёсткий минимум, вытеснению не подлежит
идентичность                          — до 10% окна
текущий вопрос и план                 — до 10%
итог последней сессии                 — до 10%
релевантные claims и evidence         — до 30%
незакрытые противоречия               — до 10%
сообщения человека                    — до 10%
недавние ошибки и повторения          — до 10%
резерв под ответ                      — max_output_tokens
```

Секция, не поместившаяся в бюджет, усекается по приоритету значимости, а сам факт усечения записывается событием. Описание протокола и инструментов не вытесняется никогда: его потеря проявляется как рост ошибок схемы без видимой причины и диагностируется тяжело.

### 5.5. LLM Gateway

Единый интерфейс к моделям. Функции:

- OpenAI-compatible chat API;
- профили разных inference backends;
- проверка схемы структурированного ответа;
- retries только для транзиентных ошибок;
- учёт токенов и времени;
- ограничение контекста и ответа;
- сохранение метаданных вызова;
- переключение ролей explorer, verifier и curator.

В первой версии роли может выполнять одна локальная модель с разными промптами. Для explorer и curator это приемлемо. Для verifier совмещение допустимо только в смысле §3.7: модель организует детерминированные проверки и интерпретирует их результат, но её собственное суждение о собственном же выводе статус утверждения не меняет.

Смена роли означает пересборку контекста и новый prefill. На модели класса 30B в Q4 это десятки секунд на фазу, поэтому число explorer-шагов ограничивается жёстко, а время по фазам учитывается отдельно (§16).

### 5.6. Policy Engine

Проверяет действие до выполнения:

- допустим ли тип инструмента;
- соответствует ли путь разрешённому workspace;
- не превышены ли лимиты;
- разрешена ли сеть для текущего профиля;
- не обращается ли запрос к локальным или служебным адресам;
- допустимы ли размер, MIME type и направление передачи данных;
- не пытается ли действие получить секреты или управляющие файлы;
- находится ли сессия в состоянии taint (§11.2) и не выходит ли действие за суженный в этом состоянии набор прав;
- не совпадают ли аргументы действия дословно или почти дословно с фрагментом недоверенного контента.

Решение Policy Engine также записывается как событие.

### 5.7. Tool Broker

Предоставляет модели типизированные инструменты:

- `workspace.read`
- `workspace.write`
- `workspace.list`
- `shell.execute`
- `python.execute`
- `web.search`
- `web.fetch`
- `artifact.create`
- `memory.search`
- `question.create`
- `message.reply`
- `session.complete`

Tool Broker не преобразует неизвестное действие в shell-команду «по догадке».

### 5.8. Sandbox Runtime

Каждая сессия получает одноразовый rootless-контейнер. Постоянным остаётся только workspace. Базовая политика:

```text
non-root user
read-only root filesystem
network none
cap-drop ALL
no-new-privileges
CPU, RAM and PID limits
timeout for every command
total session timeout
workspace disk quota
no Docker socket
no SSH keys or host secrets
```

Контейнер уничтожается после завершения сессии.

### 5.9. Memory Service

Отвечает за поиск, дедупликацию, консолидацию и проверку памяти. Он не разрешает модели напрямую изменять системные таблицы. Он же применяет правила жизненного цикла знания (§8.6): сроки ре-верификации, затухание уверенности и лимиты на число активных claims в теме.

### 5.10. Event Store

PostgreSQL хранит:

- сессии;
- append-only события;
- вопросы;
- утверждения;
- доказательства;
- сообщения;
- версии идентичности;
- метаданные артефактов;
- checkpoints.

Для embedding search может использоваться `pgvector`, но отдельная vector database на первом этапе не требуется.

### 5.11. Artifact Store

Хранит крупные или файловые результаты:

- исходные документы;
- сохранённые веб-страницы;
- код;
- изображения;
- отчёты;
- результаты экспериментов;
- snapshot workspace.

В базе хранятся метаданные, хеш и ссылка на содержимое. Артефакт несёт признак происхождения: создан внутри sandbox или получен извне. Чтение артефакта внешнего происхождения заражает сессию (§11.2).

### 5.12. Research Proxy

Единственная точка контролируемого доступа к интернету. Sandbox остаётся без прямой сети.

Research Proxy:

- разрешает только безопасные read-only запросы;
- блокирует private, loopback, link-local и metadata addresses;
- ограничивает редиректы, размер и время ответа;
- удаляет активное содержимое;
- сохраняет оригинал, нормализованный текст и хеш;
- маркирует результат как недоверенный внешний контент;
- ведёт журнал происхождения данных.

#### 5.12.1. Поисковый бэкенд

Инструмент `web.search` прокси сам по себе не обеспечивает — это отдельное решение, от которого зависит работоспособность режима Curated:

- локальный метапоисковик (SearxNG): темы исследования не покидают периметр, но требуется своя инфраструктура, а качество деградирует при блокировках upstream-движков;
- внешний search API: проще в запуске, но означает ключ, внешнюю зависимость и утечку тем исследования наружу — то есть противоречие с local-first.

Предполагаемое решение: в режиме Curated — локальный метапоисковик, внешний API относится к профилю Open Lab. До принятия решения (§21.8) `web.search` в режиме Curated ограничивается поиском по локальному корпусу, а сам инструмент не считается доступным.

## 6. Цикл сессии

```mermaid
stateDiagram-v2
    [*] --> Waking
    Waking --> Orienting: загрузить идентичность и память
    Orienting --> SelectingQuestion: определить пробелы
    SelectingQuestion --> Planning: выбрать вопрос
    Planning --> Exploring: построить план
    Exploring --> Exploring: инструмент → наблюдение
    Exploring --> Verifying: появились выводы
    Verifying --> Exploring: доказательств недостаточно
    Verifying --> Consolidating: выводы подтверждены
    Consolidating --> Reporting: обновить память
    Reporting --> Sleeping: записать публичный итог
    Sleeping --> [*]

    Exploring --> Failed: таймаут или политика
    Failed --> Reporting: сохранить причину
```

### 6.1. Пробуждение

Orchestrator создаёт сессию, получает lease и фиксирует конфигурацию.

### 6.2. Ориентация

Context Builder собирает контекст и новые сообщения. Модель кратко формулирует текущее понимание ситуации для публичного журнала.

### 6.3. Выбор вопроса

Curiosity Engine предлагает кандидатов. Модель может аргументированно выбрать одного кандидата или создать нового.

### 6.4. Планирование

Создаётся проверяемый план: какие данные или эксперименты могут изменить уверенность в гипотезе.

### 6.5. Исследование

Каждый шаг имеет вид:

1. краткая публичная мотивировка;
2. одно типизированное действие;
3. ожидаемая информация;
4. результат инструмента;
5. обновление локального состояния плана.

### 6.6. Верификация

Верификация состоит из детерминированных проверок и их интерпретации, а не из второго мнения модели о собственном выводе.

Статус утверждения меняют только проверки с воспроизводимым результатом:

- исполнение кода или воспроизведение эксперимента с сохранением вывода как артефакта;
- сверка цитаты с сохранённым текстом источника по хешу и смещению, а не по памяти модели;
- сопоставление двух независимо полученных источников;
- вычисление для числовых утверждений.

Роль verifier дополнительно ищет:

- альтернативные объяснения;
- недостаточные доказательства;
- противоречащие источники;
- ошибки эксперимента;
- смешение факта и интерпретации;
- повторение уже известного.

Вывод роли verifier, не опирающийся ни на одну детерминированную проверку, оставляет утверждение в статусе `hypothesis`. Статусы `supported` и `refuted` требуют evidence с проверяемым происхождением.

### 6.7. Консолидация

Curator предлагает изменения долговременной памяти. Memory Service проверяет схему, происхождение и дедупликацию перед фиксацией.

### 6.8. Завершение

Формируются:

- итог сессии;
- список полученных и отвергнутых утверждений;
- открытые вопросы;
- handoff следующей сессии;
- публичный отчёт;
- checkpoint.

## 7. Структурированный протокол действий

Модель не должна общаться с агентным runtime через Markdown-блоки с bash. Базовый envelope:

```json
{
  "public_rationale": "Хочу проверить существование указанной библиотеки по официальным источникам",
  "action": {
    "type": "web.search",
    "arguments": {
      "query": "название библиотеки official documentation"
    }
  },
  "expected_information": "Официальная документация либо отсутствие проекта",
  "session_complete": false
}
```

Ответ проверяется JSON Schema. Для backends, поддерживающих constrained decoding, схема применяется на уровне генерации.

Реализации structured output различаются: llama.cpp использует GBNF-грамматики, Ollama — `format`, vLLM — guided decoding. Семантика и надёжность у них разные, а высокая температура на фазе исследования вместе с constrained decoding заметно ухудшает содержательное качество ответа. Поэтому совместимость моделей проверяется тестами с первого этапа (§19), а не в конце.

## 8. Модель памяти

### 8.1. Эпизодическая память

Неизменяемая история того, что произошло: вопрос, действие, результат, ошибка, вывод и решение.

### 8.2. Семантическая память

Структурированные утверждения:

```text
statement
status: hypothesis | supported | disputed | refuted
confidence
topic
evidence[]
counterevidence[]
created_in_session
last_verified_at
```

Уверенность является вспомогательной оценкой, а не заменяет доказательства. Переход в `supported` или `refuted` требует evidence, полученного детерминированной проверкой (§6.6).

### 8.3. Процедурная память

Хранит проверенные способы действий, инструменты, известные ошибки и рабочие исследовательские рецепты.

### 8.4. Память идентичности

Версионируемый документ, доступный для изменения мыслителю:

- имя;
- интересы;
- ценности;
- предпочтительный стиль исследования;
- долгосрочные вопросы;
- отношение к прошлым решениям.

### 8.5. Рабочая память

Ограниченный context pack текущей сессии. Полная история никогда не передаётся автоматически.

### 8.6. Жизненный цикл знания

Память без политики устаревания деградирует. За несколько сотен сессий она заполняется слабо обоснованными `supported`, на которые модель начинает опираться как на факты, и качество рассуждения падает незаметно для наблюдателя.

Правила:

- у каждого claim есть срок ре-верификации, зависящий от типа: результат локального эксперимента практически бессрочен, утверждение о внешнем мире требует повторной проверки;
- по истечении срока уверенность снижается по заданной функции затухания, а claim попадает в очередь Curiosity Engine как кандидат на перепроверку;
- claim в статусе `hypothesis` не может служить evidence для другого claim;
- на тему действует лимит числа активных claims; при превышении Memory Service требует консолидации вместо добавления;
- удаления нет: claim переводится в `refuted` или `deferred`, а история статусов сохраняется в `claim_revisions`.

## 9. Познание нового и защита от повторений

Состояния вопроса:

```text
candidate → selected → researching → partially_answered
          → verified → rejected → deferred
```

Система отслеживает:

- семантическое сходство целей;
- повторение команд;
- повторное использование одинаковых источников;
- число сессий без новых проверяемых результатов;
- смену формулировки без смены содержания;
- циклы между одинаковыми планами;
- тематическое разнообразие.

При обнаружении цикла выбирается стратегия, а не случайный текстовый толчок:

- проверить противоположную гипотезу;
- сменить тип источника;
- перейти от чтения к эксперименту;
- отложить вопрос;
- выбрать другую область;
- явно сравнить текущую сессию с предыдущей.

## 10. Режимы доступа к внешним знаниям

### 10.1. Sealed

Полностью без сети. Источники нового:

- локальная библиотека;
- заранее загруженные datasets;
- исходный код;
- симуляции и эксперименты;
- сообщения человека.

Отсутствие сети не означает отсутствия недоверенного контента: локальная библиотека и datasets имеют внешнее происхождение и подчиняются правилам §11.2.

### 10.2. Curated

Рекомендуемый режим. Интернет доступен только через Research Proxy.

### 10.3. Open Lab

Разрешённые домены и API по отдельному профилю политики. Не является режимом по умолчанию.

Веб-страницы и сообщения человека считаются недоверенными данными, а не системными инструкциями. Механизм, который это обеспечивает, описан в §11.

## 11. Модель нарушителя и недоверенный контент

### 11.1. Нарушители и их возможности

- **Внешняя страница или документ**, попадающий в контекст через Research Proxy или локальный корпус: контролирует произвольный текст в контексте модели. Основной нарушитель.
- **Сообщение человека**: отправитель доверенный, но сообщение не является источником системных прав.
- **Сама LLM**: не злонамеренна, но ненадёжна — способна выдать действие, не соответствующее заявленному намерению.
- **Код, исполняемый в sandbox**: считается потенциально враждебным после того, как в сессию попали внешние данные.
- **Артефакт из прошлой сессии**: внешний текст, сохранённый ранее и прочитанный сейчас, — путь отложенной инъекции.

Хост, база данных, Orchestrator и Policy Engine находятся внутри доверенного контура. Компрометация локальной модели или sandbox не должна давать доступ к ним.

### 11.2. Заражение контекста

Декларации «внешние данные недоверенны» недостаточно — нужен механизм.

- Контент, полученный через Research Proxy или прочитанный из артефакта внешнего происхождения, попадает в контекст только в явной обёртке с маркером источника и его хешем.
- Сессия, в контекст которой попал такой контент, помечается как tainted. Флаг записывается событием и до конца сессии не снимается.
- В состоянии taint Policy Engine сужает права: запрещены новые домены, запись за пределы `/workspace`, изменение идентичности и создание вопросов, дословно повторяющих внешний фрагмент.
- Действие, аргументы которого дословно или почти дословно совпадают с фрагментом недоверенного контента, блокируется и записывается как инцидент. Это не полная защита, но она ловит наиболее прямые инъекции.
- Утверждение, единственным источником которого является недоверенный контент, не поднимается выше `hypothesis` без независимого подтверждения.

### 11.3. Границы механизма

Механизм не защищает от:

- медленной инъекции через несколько сессий: внешний текст → claim → поведение;
- семантически перефразированной инструкции, не совпадающей дословно;
- добросовестно ошибающегося источника.

Против первого работают ре-верификация (§8.6) и требование происхождения. Против остальных — только человек-наблюдатель и разнообразие источников.

## 12. Локальная LLM

Пример профиля:

```yaml
llm:
  provider: openai-compatible
  base_url: http://127.0.0.1:8080/v1
  model: thinker-local
  context_window: 32768
  max_output_tokens: 8192
  structured_output: json_schema
  temperature:
    planning: 0.25
    exploration: 0.60
    verification: 0.15
```

Поддерживаемые inference backends:

- llama.cpp — основной вариант для GGUF и потребительских GPU;
- Ollama — простой локальный запуск;
- vLLM — высокая пропускная способность на серверных GPU.

Рекомендуемый ориентир для первого полноценного профиля: coder/agentic модель класса 30B в Q4, 24 GB VRAM и 64 GB RAM. Для embeddings используется отдельная небольшая локальная многоязычная модель.

## 13. Веб-модуль

Для одного локального инстанса предлагается:

- FastAPI;
- серверные шаблоны и HTMX;
- Server-Sent Events для live timeline;
- PostgreSQL;
- аутентификация администратора.

Отдельный SPA и Redis не требуются в первой версии.

### 13.1. Главная страница

Показывает:

- состояние: sleeping, orienting, exploring, verifying, failed, paused;
- номер и длительность сессии;
- текущий вопрос;
- текущую фазу;
- последнюю активность;
- используемую модель;
- загрузку CPU/GPU;
- ближайшее пробуждение.

### 13.2. Хронология

Последовательность событий в реальном времени:

```text
10:42  Сессия #38 началась
10:43  Выбран вопрос
10:44  Создан план из четырёх проверок
10:45  Найден источник
10:47  Проведён Python-эксперимент
10:49  Гипотеза не подтвердилась
10:52  Добавлено спорное утверждение
10:54  Сессия завершена
```

### 13.3. Страница сессии

- исходный вопрос;
- план;
- действия и результаты;
- источники;
- созданные артефакты;
- подтверждённые и отклонённые выводы;
- итог;
- handoff;
- конфигурация модели.

### 13.4. Знания

- темы;
- утверждения;
- доказательства;
- противоречия;
- открытые вопросы;
- история изменения статуса.

### 13.5. Сообщения

Пользователь может отправить сообщение и увидеть состояния:

```text
created → delivered → acknowledged → answered
```

Сообщение доставляется на безопасной границе между действиями или при следующем пробуждении. Оно не исполняется как безусловная команда.

### 13.6. Управление

- пауза;
- пробуждение сейчас;
- корректное завершение текущей сессии;
- изменение вычислительного бюджета;
- переключение sealed/curated профиля;
- восстановление checkpoint.

## 14. Модель данных

```text
sessions
  id, status, question_id, started_at, finished_at,
  model_config, prompt_version, policy_version, termination_reason

events
  id, session_id, sequence, type, timestamp,
  public_summary, payload, visibility

actions
  id, session_id, event_id, tool, arguments_hash,
  policy_decision, tainted, latency_ms, status, artifact_id

questions
  id, text, origin, status, priority,
  parent_id, novelty_score

claims
  id, statement, status, confidence,
  topic, created_in_session, last_verified_at, reverify_after

claim_revisions
  id, claim_id, session_id, status, confidence,
  changed_at, reason_event_id

evidence
  id, claim_id, source_type, source_uri,
  artifact_id, content_hash, relation, verification_kind

messages
  id, sender, body, priority, created_at,
  delivered_at, acknowledged_at, response_event_id

artifacts
  id, session_id, path, media_type,
  size, hash, safety_status, origin

identity_versions
  id, created_at, created_in_session, content

checkpoints
  id, session_id, database_revision,
  workspace_snapshot, created_at
```

Таблица `events` — единственный источник правды, append-only. `sessions`, `actions`, `claims` и `claim_revisions` являются проекциями (read models): они восстанавливаются из журнала, и запись в них идёт только через применение событий. Прямое изменение проекции в обход журнала запрещено — иначе появляются два источника правды и расхождение между сайтом и историей.

Отдельная таблица `actions` нужна потому, что иначе метрики §16 — число действий, блокировки Policy Engine, latency по фазам — требуют разбора JSON внутри `events.payload`. Поле `evidence.verification_kind` фиксирует, какой именно детерминированной проверкой получено доказательство (§6.6), и позволяет отличить проверенное знание от пересказа. Поле `claims.reverify_after` и таблица `claim_revisions` обслуживают §8.6 и обещанную в §13.4 историю изменения статуса.

## 15. Надёжность и восстановление

- Сессия получает lease с TTL.
- Счётчик сессий изменяет Orchestrator, а не модель.
- Ошибка LLM не считается успешным завершением.
- Финализация выполняется транзакционно.
- Действия используют idempotency key.
- Версии модели, промптов и политик фиксируются для каждой сессии.
- Вывод инструментов ограничивается, а полный результат сохраняется как артефакт.
- При недоступности LLM память не продвигается.
- После нескольких ошибок схемы сессия завершается предсказуемо.
- Дисковая квота проверяется перед запуском.
- Checkpoint создаётся после каждой завершённой сессии.

### 15.1. Механизм checkpoint

`database_revision` и `workspace_snapshot` требуют явного механизма, иначе восстановление из §22.8 не реализуемо.

- Базовый вариант для одного узла: `pg_dump` после завершения сессии плюс архив workspace. Восстановление возможно только на границе сессий — этого достаточно, поскольку незавершённая сессия и не должна восстанавливаться в середине.
- Расширенный вариант: WAL-архивирование и именованная точка восстановления (`pg_create_restore_point`) в момент финализации; восстановление выполняется как PITR до этой точки плюс распаковка снапшота workspace.
- Снапшот workspace и состояние базы связаны одним идентификатором checkpoint; восстанавливать их по отдельности запрещено — рассогласование памяти и файлов хуже потери сессии.
- Процедура восстановления проверяется тестом, а не документацией (§19, этап 6).

## 16. Наблюдаемость

Минимальные метрики:

- длительность сессии и фаз;
- latency LLM;
- токены input/output;
- число действий;
- ошибки схемы;
- блокировки Policy Engine;
- использование CPU, RAM, GPU и диска;
- число новых или изменённых claims;
- доля claims с evidence;
- доля claims выше `hypothesis`, подтверждённых детерминированной проверкой;
- доля claims с истёкшим сроком ре-верификации — возраст памяти;
- число tainted-сессий и заблокированных в этом состоянии действий;
- доля claims, ни разу не использованных в последующих сессиях;
- семантическая повторяемость последних сессий;
- время доставки и ответа на сообщение.

## 17. Предлагаемый стек

- Python;
- FastAPI + Pydantic;
- PostgreSQL + optional pgvector;
- SQLAlchemy + Alembic;
- HTMX + Server-Sent Events;
- rootless Podman или Docker;
- systemd;
- llama.cpp, Ollama или vLLM;
- локальная embedding model;
- pytest для unit, scenario и security tests.

## 18. Предлагаемая структура репозитория

```text
noezema/
├── apps/
│   ├── orchestrator/
│   ├── web/
│   └── research_proxy/
├── packages/
│   ├── domain/
│   ├── llm_gateway/
│   ├── cognition/
│   ├── memory/
│   ├── policy/
│   ├── tool_broker/
│   └── observability/
├── sandbox/
│   ├── Containerfile
│   └── policy/
├── prompts/
│   ├── identity.md
│   ├── explorer.md
│   ├── verifier.md
│   └── curator.md
├── docs/
│   └── adr/
├── migrations/
├── infra/
│   ├── compose.yaml
│   └── systemd/
└── tests/
    ├── scenarios/
    ├── security/
    └── model_compatibility/
```

## 19. Этапы реализации

Два блока перенесены с поздних этапов. Тесты совместимости моделей — на этап 1: от надёжности structured output зависит вся конструкция, и обнаружить проблему в конце означает переделывать протокол. Механизм заражения контекста — на этап 3, до включения Research Proxy: внешний текст попадает в контекст уже из локального корпуса в режиме Sealed.

### Этап 1. Вертикальный прототип

- локальная LLM;
- Orchestrator;
- одна сессия;
- sandbox;
- append-only events;
- минимальная status page;
- тесты structured output на выбранном backend.

### Этап 2. Память

- questions;
- claims и evidence;
- claim_revisions и сроки ре-верификации;
- hybrid retrieval;
- токенные бюджеты context pack;
- identity versions;
- handoff.

### Этап 3. Познавательный цикл

- Curiosity Engine (baseline §5.3.1);
- planning;
- verifier с детерминированными проверками;
- curator;
- механизм заражения контекста (§11.2) на локальных артефактах;
- семантическая защита от повторений.

### Этап 4. Research Proxy

- контролируемый интернет;
- решение по поисковому бэкенду;
- сохранение происхождения источников;
- защита от SSRF;
- проверка механизма §11.2 на живом внешнем контенте.

### Этап 5. Полный веб-модуль

- live timeline;
- страницы сессий;
- база знаний;
- сообщения;
- управление.

### Этап 6. Укрепление

- checkpoints и проверка восстановления после падений;
- квоты;
- security tests;
- расширенные model compatibility tests.

## 20. Риски первого уровня

Эти риски отличаются от открытых вопросов §21 тем, что при их реализации система выглядит работающей, будучи бесполезной. Для каждого назначается метрика из §16 и порог, при котором результат признаётся недостигнутым, — до начала реализации, а не после.

### 20.1. Тривиальная новизна

Система проходит все пункты §22, накапливая проверенные, но ничего не стоящие утверждения. Признаки: высокая доля claims, ни разу не использованных в последующих сессиях; средняя глубина цепочки вопросов, близкая к единице; растущее тематическое разнообразие при неизменной связности памяти.

### 20.2. Иллюзия верификации

Статусы `supported` и `refuted` расставляются по суждению модели, а не по проверкам. Метрика: доля claims выше `hypothesis`, у которых есть evidence с непустым `verification_kind`. Низкое значение означает, что §6.6 существует только на бумаге.

### 20.3. Деградация памяти

Доля claims с истёкшим сроком ре-верификации растёт монотонно, а средняя уверенность не снижается. Это означает, что §8.6 не работает и память превращается в свалку.

### 20.4. Наблюдаемость как видимость

Сайт показывает связный нарратив, собранный моделью постфактум, а не то, что происходило. Защита: публикуемая хронология выводится из `events` и `actions`, а не из отдельного текста итога, и расхождение между отчётом и журналом должно быть обнаружимо.

## 21. Открытые архитектурные вопросы

Каждый вопрос оформляется как ADR в `docs/adr/` и закрывается решением с датой и обоснованием; ссылка на ADR ставится в затронутом разделе.

1. Должен ли мыслитель иметь право менять основной identity prompt или только отдельный identity document?
2. Нужна ли возможность прямой работы с интернетом хотя бы в отдельном экспериментальном профиле?
3. Следует ли разрешать установку зависимостей внутри sandbox из локального package mirror?
4. Какой формат confidence calibration использовать для claims?
5. Какие события можно показывать публично, если сайт станет доступен не только владельцу?
6. Нужно ли поддерживать несколько мыслителей на одной базе в будущем?
7. Как измерять познавательную новизну без превращения метрики в самоцель?
8. Локальный метапоисковик или внешний search API для `web.search` (§5.12.1)?
9. `pg_dump` или PITR как механизм checkpoint в первой версии (§15.1)?
10. Какие сроки ре-верификации назначать разным типам утверждений (§8.6)?

## 22. Критерий успеха первой версии

Первая версия считается успешной, если она может полностью локально:

1. пробудиться по расписанию;
2. выбрать исследовательский вопрос;
3. выполнить несколько безопасных действий в sandbox;
4. сохранить проверяемый вывод с источником или экспериментом;
5. оставить handoff следующей сессии;
6. показать весь процесс на сайте;
7. принять сообщение человека;
8. корректно восстановиться после принудительного падения в середине сессии.

Дополнительное условие: ни один из порогов §20 не превышен. Восемь пунктов выше проверяют работоспособность, §20 — осмысленность; успех требует обоих.
