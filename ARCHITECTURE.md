...
Версия 0.25 превращает смену host recovery policy в однозначную crash-safe машину состояний и ограничивает jitter реальным MVP-контрактом:

- у смены policy появился единственный `host-policy-change-head.json`; normal install и новый host transition запрещены, пока он активен;
- terminal outcomes различают `committed`, `aborted` и operator `resolved` и всегда сохраняют фактический `effective_hash`;
- валидный третий hash можно принять явно, а невалидный файл заменить через `resolve-host-policy --file` без второго `prepared`;
- schema v1 single-node профиля требует `resume_retry_jitter=0.0`; multi-thinker semantics отложена до отдельного ADR/schema version.
...
Версия 0.23 делает host recovery однозначным и проверяемым даже при retained history, падениях между fsync и неточной диспетчеризации systemd:

- введён единственный durable `host-transition-head.json`: он сериализует незавершённый переход, служит O(1)-индикатором scheduler-а и восстанавливается из журнала после crash;
- host recovery policy получила единственный root-owned источник, versioned schema, канонический hash и явный протокол безопасного изменения; DB snapshot больше не может неявно менять её;
- короткий retry получения flock ограничен конкретными числами, а flock удерживается только на host-journal critical sections;
- retry и unit-state timers получили явные `AccuracySec=1s`/`RandomizedDelaySec=0`, failpoints, метрики и критерии приёмки.
...
Host maintenance принадлежит `noezema-offline-rules.service`, а не произвольному процессу. Unit получает `flock` на стабильном `/run/lock/noezema-host-transition.lock` с общим deadline 2 секунды: первая пауза 50 ms, затем exponential backoff с cap 250 ms и jitter ±10%. Timeout до захвата возвращает временный exit `75` и не меняет marker, current record или active head. После захвата helper сначала reconciles active head (§8.7.1.1): существующий unresolved transition даёт typed `host_transition_in_progress`, а не новый retry. Только при свободном head создаются initial event/current/head и owner-bound marker.

Flock защищает лишь короткие чтения и fsync-safe изменения host journal; сетевые вызовы, PostgreSQL и `systemctl` под ним не выполняются. После публикации head и marker lock освобождается, а единственность долгой операции обеспечивает active head вместе с systemd ownership marker. `RuntimeDirectory=noezema-offline-rules` создаёт private каталог, marker публикуется atomic rename как `/run/noezema-offline-rules/active`. Второй запуск завершается до изменения marker; helper напрямую вне unit запускать запрещено. Только владеющий unit может снять marker.
...
Condition не является correctness-проверкой: systemd может cleanly skip unit, а уже запущенный процесс condition не остановит. Поэтому target и каждый напрямую запускаемый member также имеют `Requires=noezema-runtime-admission.service` и `After=noezema-runtime-admission.service`. Это `Type=oneshot`, `RemainAfterExit=no`: один coalesced start job получает `/run/lock/noezema-host-transition.lock`, проверяет отсутствие marker, global runtime-head tuple, bootstrap seed hash, host-transition state и отсутствие nonterminal host-policy-change head, при необходимости reconciles/replay-ит/закрывает journals и затем освобождает flock. Любая ошибка возвращает non-zero, поэтому required target/member не стартует. Прямой `systemctl start` отдельного member-а запускает тот же admission и не создаёт обхода.
...
До публикации marker maintenance под host-transition flock создаёт current record `/var/lib/noezema/host-transitions/<attempt_id>.json` с mode `0640`, owner `root:noezema-observer`. Запись использует temporary file → `fsync(file)` → atomic rename → `fsync(directory)`. Record содержит `attempt_id`, operation, candidate/base snapshot IDs, observed pointer tuple, `state`, `attempts_total`, `current_attempt_seq`, `consecutive_unclassified_failures`, `backoff_step`, `error_class`, `last_probe_started_at`, `last_probe_classified_at`, `next_attempt_at`, `dispatch_id`, `dispatch_deadline`, `last_event_seq`, `replayed_through_seq`, `replayed_at` и материализованный host-policy contract ниже. Конечного `max_attempts` для транзиентного recovery нет.

Единственный authoritative индекс незавершённого перехода — `/var/lib/noezema/host-transition-head.json` с теми же owner/mode и fsync-safe publication. Он содержит `schema_version`, `attempt_id`, `record_identity_sha256`, `created_at` и `creation_boot_id`; identity hash вычисляется только по immutable header `{schema_version, attempt_id, operation, created_at, initial_event_sha256}` и не меняется при обновлении mutable current state. Наличие head означает, что новый host transition запрещён; retained `resolved` records и каталоги events не влияют на fast path scheduler-а.

Создание выполняется под одним flock в порядке initial event → current record → active head → marker. После crash между current и head initial resume/admission один раз сканирует records: ноль unresolved оставляет head отсутствующим, один восстанавливает его, больше одного фиксирует permanent inconsistency и запрещает runtime start. Head на отсутствующий, повреждённый или другой record также даёт `resume_blocked`; head на уже resolved record завершается проверкой полного replay и удаляется. Этот reconciliation выполняется при boot и перед созданием перехода, но не на каждом пустом timer tick.
...
Durable backoff policy задаётся отдельно от БД. Packaged baseline `/usr/lib/noezema/host-recovery.defaults.toml` устанавливается вместе с юнитами и проверяется в CI; необязательный override — `/etc/noezema/host-recovery.toml`. Оба файла имеют `schema_version=1`, являются обычными non-symlink файлами `root:root 0644` без secrets, не могут быть group/world-writable и читаются через descriptor с проверкой owner/mode/inode. Отсутствие override означает packaged baseline, но отсутствие либо невалидность baseline является ошибкой установки.

Минимальная схема policy:
...
Policy меняется только root-командой `noezemactl install-host-recovery-policy --file ... --reason ...`. Единственный authoritative индекс незавершённого изменения — `/var/lib/noezema/host-policy-change-head.json`, fsync-safe файл `root:noezema-observer 0640` с `schema_version`, `change_id`, `prepared_event_sha256`, old/proposed source и canonical hashes, `created_at`. Retained terminal event streams не считаются active.

Под коротким host flock installer **до** нового `prepared` reconciles policy head. При отсутствующем head boot/installer scan восстанавливает его из единственного orphan nonterminal stream; больше одного такого stream или head/event mismatch является permanent journal inconsistency. Пока head существует, normal installer и создание любого host transition завершаются typed `host_policy_change_in_progress`: второй `prepared` невозможен.
...
validate candidate
  ...
  → publish terminal:
       committed(effective_hash=proposed_hash)  # файл равен proposed
       aborted(effective_hash=old_hash)         # файл остался old
  → unlink head + fsync(parent directory)
```

Event stream лежит в `/var/lib/noezema/host-policy-events/<change_id>/<event_seq>.json` и использует тот же immutable/fsync-safe protocol, что host transitions. Crash после event, но до head, восстанавливается scan-ом; crash после terminal, но до unlink — terminal cleanup. Head удаляется только после fsync terminal event с обязательным `effective_hash`.

Если фактический hash не равен ни old, ни proposed, recovery публикует nonterminal `inconsistent(observed_hash)`, оставляет head и alert. Прямое редактирование override вне `noezemactl` считается unsupported out-of-band mutation, а не штатным способом настройки. Выход выполняется только для указанного active `change_id`:

```text
noezemactl resolve-host-policy --accept-current --reason ...
noezemactl resolve-host-policy --file <valid-policy> --reason ...
```

`--accept-current` разрешён только для валидного текущего файла и публикует terminal `resolved(resolution_action=accept_current, effective_hash=current_hash)` — не `aborted`. `--file` работает и при невалидном текущем файле: валидирует replacement, в **том же** stream публикует `resolution_prepared(base_observed_hash, replacement_hash)`, атомарно заменяет override и публикует `resolved(resolution_action=install_replacement, effective_hash=replacement_hash)`. Crash до rename оставляет stream nonterminal и допускает идемпотентный повтор команды; crash после rename завершается сравнением replacement hash. Обычный installer при active head по-прежнему запрещён.

Таким образом `committed` всегда означает proposed hash, `aborted` — old hash, а `resolved` — явно выбранный оператором валидный третий/replacement hash. Все terminal events содержат actor, reason и `effective_hash`, идемпотентно replay-ятся в DB audit; только после их fsync снимается head и разрешается следующий transition. Config snapshots могут хранить наблюдаемый hash для диагностики, но **не являются источником** и автоматически не продвигаются в host policy. Новая policy вступает в силу только для следующего transition.

`noezema-runtime-resume-retry.timer` имеет явную точность; domain jitter в schema v1 равен нулю, systemd jitter также отключён. В MVP `retry_timer_period=30s` и `retry_timer_accuracy=1s` pin-ятся unit-файлом: policy validator сверяет их с `systemctl show` при boot/admission, а override с другими значениями отклоняется. Их изменение требует согласованного package/unit upgrade, не редактирования DB:
...
Backoff квантуется `retry_timer_period` и дополнительно запаздывает не более чем на `retry_timer_accuracy` при исправном scheduler-е. Для MVP single-node schema v1 требует `resume_retry_jitter=0.0`: декоррелировать нечего, а значение меньше timer quantum даёт фазозависимую задержку. Универсальное правило вида `initial * jitter >= timer_period` намеренно не задаётся. Ненулевой jitter появится только в новой schema version после ADR для multi-thinker (§21.6), который определит общую зависимость, фазу timers и способ reschedule.

Scheduler сначала делает без блокировки `stat("/var/lib/noezema/host-transition-head.json")`. `ENOENT` завершает тик без flock и без сканирования retained history. При наличии head service получает flock, валидирует head/current/event prefix и `next_attempt_at`; только state `retry_wait | resume_degraded` с due deadline может атомарно получить новый `dispatch_id`. Затем lock освобождается и запускается `noezema-runtime-resume.service`.
...
`noezema-runtime.target` напрямую при boot не enable-ится: `multi-user.target` запускает web, unit-state timer, retry timer и один initial resume probe; уже успешный resume активирует cognitive runtime target. После boot reconciliation отсутствие и host-transition head, и nonterminal host-policy-change head позволяет probe создать `operation=runtime_start`; retained terminal records этому не мешают, поэтому cold boot использует тот же journal/audit protocol.

Admission/resume replay-ит immutable host events в `audit_events` с unique key `(attempt_id, event_seq)` и в той же DB-транзакции создаёт terminal resolution audit event. После подтверждённого commit current record fsync-safe получает `replayed_through_seq=last_event_seq`, `replayed_at` и `resolved`. Затем под flock проверяются identity hash/current и active head удаляется через `unlink → fsync(parent directory)`. Падение после `resolved`, но до удаления head безопасно: следующий resume видит terminal record и завершает cleanup; удаление head до полного replay запрещено. Current record и event directory остаются recovery/GC roots до `resolved + complete replay + retention`, active head — до этого terminal cleanup.
...
`noezema-web.service` не входит в `noezema-runtime.target`, не имеет `Requires/PartOf` на cognitive runtime или PostgreSQL и продолжает работать во время offline maintenance. DB connector инициализируется lazy. Web переходит в read-only degraded mode и закрывает Command API при active host-transition head со state `checking | retry_wait | ready_to_start | resume_degraded | resume_blocked`, nonterminal host-policy-change head, любой обнаруженной head/journal inconsistency, active maintenance marker, неактивном/нездоровом runtime target/member, недоступной DB либо missing/stale unit-state snapshot. Доступная DB projection при host degradation остаётся read-only и явно отделяется от локального host status; недоступная projection маркируется как unavailable. ACL даёт web-процессу только чтение обоих heads, current records, host-transition и host-policy immutable events; менять state либо запускать units через сайт запрещено.

Состояние юнитов web не читает с system bus напрямую: песочница веб-сервиса не имеет доступа к `org.freedesktop.systemd1`. Отдельный publisher запускается коротким timer с явной точностью:
...
AccuracySec=1s
RandomizedDelaySec=0
Unit=noezema-unit-state.service
```

Только `noezema-unit-state.service` выполняет `systemctl show --property=ActiveState,SubState,Result` для target и его непустого `ConsistsOf`. Publisher создаёт `/run/noezema/unit-state.json` через temporary file → `fsync(file)` → rename → `fsync(directory)`, owner `root:noezema-observer`, mode `0640`.

Snapshot содержит `schema_version`, `boot_id`, `observed_at`, `publisher_result`, target state, отсортированный member list, `inventory_sha256` и состояние каждого unit. Web принимает его только при совпадающем `boot_id`, корректном schema/inventory и age ≤15 секунд; нормативно TTL должен быть не меньше `2 × period + 2 × AccuracySec` (для defaults это 12 секунд). Иначе web показывает `unknown`, закрывает Command API и поднимает degraded banner. Метрика timer lateness сравнивает фактический monotonic start с ожидаемым deadline; `AccuracySec=1s` не заменяет эту проверку.
...
- host-policy change `prepared | inconsistent | resolution_prepared | committed | aborted | resolved`, effective hash и operator resolution action.

SSE получает committed outbox events и отдельные host-transition/host-policy notifications без представления локального record как DB audit до replay. Staging knowledge не публикуется как действующее; discarded/failed diagnostics доступны владельцу в отдельном visibility-классе.
...
Перепроверка существующего claim'а (сессия сверяет установленный факт со свежими источниками, не изменяя значения claim'а) записывается отдельной строкой `claim_assessments`, привязанной к сессии: `claim_id` + `created_in_session` = сессия-перепроверка — новый момент проверки по правилу §8.6 (ADR-0017), существующий независимо от того, появилось ли новое evidence (повторно загруженный идентичный источник переиспользует строку evidence якоря по identity, §14.3). Строка создаётся тем же путём, что и оценка нового/переиспользованного claim'а; строки reassessment worker и активации имеют `created_in_session IS NULL` и в сессионную запись не входят. Ссылка модели на существующий claim — по host-issued id (claim-операция `session_staging` с `existing_claim_id`; префикс разрешается только при однозначности среди видимых сессии claim'ов, fail-closed). Перепроверка — не дедупликация (та же строка claim'а по id, а не поиск по statement+type) и не ревизия значения (`claim_revisions`, §8.7: там `previous_value → new_value`). Путь гейта 5 §22.2 «значимые claims переиспользуются/перепроверяются» включает сессионные строки `claim_assessments` (ADR-0018, T7.34).
...
- Host-transition active head и связанный current record в `checking | retry_wait | ready_to_start | resume_degraded | resume_blocked`, его immutable events и незавершённый dispatch являются recovery/GC roots. Recovery под host flock сначала reconciles head: восстанавливает его при единственном orphan unresolved record, очищает только terminal fully-replayed head и блокирует start при множественных/несогласованных records. Затем current snapshot восстанавливается по непрерывному `event_seq`, просроченный dispatch инвалидируется и classification продолжается. При согласованной БД admission/resume идемпотентно replay-ит events в `audit_events` по `(attempt_id, event_seq)`, добавляет terminal resolution event, fsync-safe переводит local record в `resolved` и только после этого удаляет active head.

Host-policy-change head и его nonterminal event stream также являются recovery/GC root. До любого runtime admission recovery восстанавливает единственный orphan head, сверяет фактический override с old/proposed/replacement hashes и либо дописывает однозначный terminal event, либо сохраняет `inconsistent` до audited operator resolution; runtime при active policy head не стартует.
...
Checkpoint — committed `{knowledge_revision, dependency_graph_revision}` плюс workspace manifest. Backup связывает DB recovery point с content-addressed artifact inventory и включает host-transition active head, host-policy-change active head, все unresolved current records вместе с immutable event directories, host-policy files, их hashes и незавершённые/retention-window host-policy event streams либо явное доказательство отсутствия обеих active operations. Restore до старта cognitive runtime запускает boot reconciliation/admission; любой из двух active heads либо обнаруженный orphan nonterminal stream открывает web в degraded mode и проходит соответствующую resume/policy-классификацию.
...
- active host-transition head и host-policy-change head — до соответствующего terminal cleanup; current records, immutable events и live dispatch metadata — до `resolved + replayed_through_seq=last_event_seq + replayed_at + retention`; policy event streams — до terminal `effective_hash`, audit replay и retention;
...
- незавершённое изменение host policy имеет ровно один fsync-safe active head и один event stream; normal install/host transition запрещены до terminal `committed | aborted | resolved`, каждый terminal содержит фактический `effective_hash`, invalid third file исправляется внутри того же `change_id`, head снимается только после terminal fsync, DB config не является источником policy, а schema v1 требует `resume_retry_jitter=0.0`;
- незавершённый host transition имеет ровно один active head, fsync-safe current record и непрерывный immutable event stream; zero head означает zero unresolved records после boot reconciliation, more-than-one unresolved — permanent inconsistency; classified transient остаётся `retry_wait`, unclassified crash-loop виден как `resume_degraded`, permanent — как `resume_blocked`, а удаление head допустимо только после `resolved` и полного идемпотентного DB audit replay по `(attempt_id, event_seq)`;
...
- offline config attempts/resumes/rebuilds/limit rejects, maintenance owner conflicts/flock wait/timeouts, marker lifetime, target membership mismatch и admission rejects; active-head presence/age, head-record identity mismatch, orphan/multiple-unresolved repairs, host-policy-change head/state/age, change reconciliation/resolution/action, terminal effective-hash mismatch, host-policy schema/hash/source/drift/validation errors, host-transition state/age/attempts, classified-transient duration, unclassified crash burst, backoff step, scheduler/dispatch lag и timer lateness против `period+AccuracySec`, event-sequence gaps, audit replay lag, `resume_degraded`/`resume_blocked` alerts, unit-state snapshot age/inventory mismatch/lateness; verification duration/digest mismatch и atomic publish latency; online activation state/cursor/cohort progress, lease age/fence takeovers/stale-write rejects, shadow-head retries, publish/terminal-cleanup latency, effective repair backlog age и нарушения `T_repair_admission`;
...
- systemd 249+ (`OnSuccess=` требует 249; `PartOf=`, `ConsistsOf`, `RuntimeDirectory=`, `ConditionPathExists=`, `RestartPreventExitStatus=`, `AccuracySec=`, `RandomizedDelaySec=` и start limits используются в host protocol §8.7.1);
...
- offline-смена rules version §8.7.1 через systemd-owned maintenance unit, `PartOf/ConsistsOf` target graph, required runtime admission, отдельные active heads для host transition и policy change, fsync-safe event journals, root-owned materialized recovery policy, точные retry/unit-state timers, durable verification seal и атомарный pointer+questions publish;
- identity/handoff.
...
Gate: duplicate evidence не повышает grade; новое counterevidence меняет effective head в том же session commit; offline rules publish оставляет старую либо полностью новую config вместе с invalid-вопросами; target stop quiesce-ит все `PartOf` members, любой start проходит admission, classified outage остаётся `retry_wait`, crash-loop виден как `resume_degraded`, retained history не выглядит active, незавершённая policy change блокирует admission и имеет audited resolution, permanent inconsistency/policy mismatch — как durable `resume_blocked`; неизвестная source lineage не создаёт ложную независимость; pending/invalid claim не подаётся как current; grade назначает только rules engine.
...
Target считается active при skipped members, writer не остановился вместе с target, start обошёл DB admission, retained history выглядит как active transition, два writer-а создали два unresolved attempts, незавершённая смена policy потеряла change identity или была закрыта как `aborted` при фактически принятом третьем hash, policy незаметно изменилась из DB snapshot, короткий systemd timer сработал с минутной точностью, `resume_degraded | resume_blocked` потерян между local events и audit либо web принял stale unit snapshot за живой runtime. Контроль: `PartOf/ConsistsOf` graph, hard-failing shared admission, отдельные единственные fsync-safe heads для host transition и policy change, boot reconciliation, terminal `effective_hash`, root-owned hash-pinned host policy, короткие flock critical sections, schema-v1 jitter 0, явные `AccuracySec=1s` timers, раздельные classified/unclassified paths, fenced retry dispatch, idempotent event replay, out-of-band alerts и TTL-проверяемый degraded observer вне cognitive runtime target.
...
33. `[MVP]` recovery policy читается только из versioned root-owned host files и hash-ируется до materialization; её смена имеет единственный active head и event stream, блокирует normal install/transition до terminal outcome с фактическим `effective_hash`, valid third hash принимается как `resolved`, invalid third file заменяется внутри того же `change_id`; missing override использует packaged baseline, DB snapshot не меняет policy, schema v1 требует jitter 0, retry/unit-state timers имеют явные `AccuracySec=1s` и тестируемую lateness;
34. `[MVP]` web остаётся вне cognitive runtime target, не имеет system-bus access и fail-closed отключает Command API при любом unresolved host transition или policy change, unhealthy runtime, DB outage либо missing/stale/mismatched unit-state snapshot; current/event chronology не выдаётся за committed DB audit и после replay дедуплицируется по event key.
